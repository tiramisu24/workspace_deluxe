 ...
 * in order to be able to read very large text values in JSON data (and store it
 * into JsonByteString nodes) and limit maximum size of these text values.
 */
public class KBaseJsonParser extends ParserBase {
    ...
    public void writeTextIntoWriter(Writer w) throws IOException {
        if (_tokenIncomplete) {
            _tokenIncomplete = false;
            _finishString(); // only strings can be incomplete
        }
        _textBuffer.contentsAsWriter(w);
    }
...
class TextBuffer
{
    ...
    public void contentsAsWriter(Writer w) throws IOException {
        if (_resultString == null) {
            // Has array been requested? Can make a shortcut, if so:
            if (_resultArray != null) {
                ...
                if (_inputStart >= 0) {
                    ...
                    if (segLen == 0) { // yup
                        if (currLen > 0)
                        	w.write(_currentSegment, 0, currLen);
                    } else { // no, need to combine
                        // First stored segments
                        if (_segments != null) {
                            for (int i = 0, len = _segments.size(); i < len; ++i) {
                                char[] curr = _segments.get(i);
                                w.write(curr, 0, curr.length);
                                _segments.set(i, null);
                            }
                        }
                        // And finally, current segment:
